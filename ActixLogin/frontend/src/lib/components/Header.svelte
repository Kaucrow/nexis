<script lang="ts">
    ...
    let darkMode = false;
    let unsubscribe = darkModeStore.subscribe(val => {
        darkMode = val;
    });
...
<header class="dark:bg-neutral-900 text-neutral-200 relative">
    <div class="container mx-auto flex items-center justify-between p-4">
        ...
        <div class="flex items-center justify-between">
            <button on:click={openSideBar} class="md:hidden relative mr-4 group">
                <svg class="dark:fill-neutral-200 w-6 h-6" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16m-7 6h7"></path>
                </svg>
                <div class="absolute top-1/2 left-1/2 w-0 h-0 bg-neutral-700 opacity-25 rounded-full transform -translate-x-1/2 -translate-y-1/2 transition-all duration-200 group-hover:w-10 group-hover:h-10"></div>
            </button>
            <svg class="fill-neutral-200 mr-2" xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 512 512"><!--!Font Awesome Free 6.6.0 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license/free Copyright 2024 Fonticons, Inc.-->
            ...
            <h1 class="text-2xl font-bold">Actix Login</h1>
            <nav class="hidden md:flex relative top-0.5 space-x-4 ml-6">
                <a href="/" class="hover:text-neutral-400 transition">Home</a>
                <a href="/auth/login" class="hover:text-neutral-400 transition">Login</a>
                <a href="/auth/register" class="hover:text-neutral-400 transition">Register</a>
                <a href="#!" class="hover:text-neutral-400 transition">About</a>
                <a href="#!" class="hover:text-neutral-400 transition">Contact</a>
            </nav>
        </div>
        ...
        <div class="flex items-center">
            ...
            <label for="toggle" class="flex items-center cursor-pointer">
                ...
                <span class="ml-3 text-neutral-800 dark:text-neutral-200">Dark Mode</span>
            </label>
        </div>
    </div>
...
<style lang="postcss">
    #toggle:checked + label > div:first-child .dot {
        transform: translateX(100%); /* Move dot to the right */
        background-color: #262626; /* Dark mode dot color */
    }
</style>